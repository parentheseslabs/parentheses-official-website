...
const FeaturedServices = () => {
    ...
    return (
        <div className='px-[7%]'>
            <h1 className='text-[4vmax] font-sfpd-bold'>Featured Services</h1>
            <section className='flex  min-h-[70svh] items-center lg:flex-row flex-col-reverse py-10 gap-16'>
                        ...
                        <div className='flex gap-4'>
                            {
                                buttons.map((item, idx) => (
                                    <button
                                        key={idx}
                                        className={`h-fit font-sfpd-semibold ${activeTab === idx || activeTab + 1 === idx ? "flex" : "hidden"} ${activeTab === idx ? "text-black" : "text-black/40"}`}
                                        onClick={() => handleClick(idx)}
                                    >{item.title}</button>
                                ))
                            }
